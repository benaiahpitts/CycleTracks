...
- (CLLocationDistance)distanceFrom:(Coord*)prev to:(Coord*)next realTime:(BOOL)realTime
{
		 ...
		 [next.hAccuracy doubleValue] < kEpsilonAccuracy )
	{
		...
		//NSLog(@"WARNING accuracy exceeds epsilon: %f => throw out deltaDist: %f", MAX([prev.hAccuracy doubleValue], [next.hAccuracy doubleValue]) , deltaDist);
	}
...
- (void)saveTrip
{
	 ...
	 */
	// we don't want anything saved unless coords exist
	if ( trip && [coords count] )
	{
		...
		UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"Error!"
														message:@"There is no trip data to save. Please log a trip with a distance > 0 miles."
